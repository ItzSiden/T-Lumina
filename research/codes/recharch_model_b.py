# ====================================================
# MODEL B: TERNARY FFN (Ternary FFN + Standard Attn)
# ====================================================
...
class AdaptiveTernaryLinear(nn.Module):
    def __init__(self, in_f, out_f, bias=False):
        super().__init__()
        self.weight_fp = nn.Parameter(torch.empty(out_f, in_f).normal_(0, 0.02))
...
print(f"Ternary FFN params: {sum(p.numel() for p in model.parameters()) / 1e6:.1f}M")
...
print(f"\n{'='*60}")
print(f"⚡ MODEL B: TERNARY FFN | {TOTAL_STEPS} steps | WikiText-2")
print(f"{'='*60}")
...
print(f"\n✅ MODEL B Training Complete! Final PPL: {best_ppl:.2f}")