...
# IMPORTANT: Ensure this path points to the original trained FP32 model (tlumina_final.pt), 
# NOT the packed model.
MODEL_PATH = "tlumina_final.pt" 
...
with open(OUTPUT, "wb") as f:
    for name, param in state.items():
        # Skip if param is not a Tensor (e.g., if you accidentally loaded a packed dict containing shapes)
        ...
        if is_ternary:
            # 1. Calculate Exact Alpha from training
            ...
            # Write Alpha Float (Type 5)
            name_a = (name + "_alpha").encode()
            f.write(struct.pack("I", len(name_a)))
            f.write(name_a)
            f.write(struct.pack("I", 5)) # Type 5 = Float
            f.write(struct.pack("I", 4)) 
            ...
            # Standard FP16 Tensor (Type 1)
            name_b = name.encode()
            f.write(struct.pack("I", len(name_b)))
            f.write(name_b)
            f.write(struct.pack("I", 1))
            fp16_tensor = param.to(torch.float16)
            data = fp16_tensor.numpy().tobytes()
            f.write(struct.pack("I", len(data)))
            f.write(data)
            print(f"➡️ Saved FP16: {name}")

print("✅ Binary Export Complete with Alpha parameters!")